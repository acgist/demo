...
public final class HTTPClient {
	 ...
	 * <p>HTTP客户端线程池</p>
	 */
	private static final ExecutorService EXECUTOR = SystemThreadContext.newCacheExecutor(SystemThreadContext.ACGIST_THREAD_HTTP_CLIENT);
	...
	public static final HTTPClient newInstance(String url) {
		return newInstance(url, CONNECT_TIMEOUT, RECEIVE_TIMEOUT);
	}
	...
	public <T> HttpResponse<T> postForm(Map<String, String> data, HttpResponse.BodyHandler<T> handler) throws NetException {
		this.builder.header("Content-type", "application/x-www-form-urlencoded;charset=" + AcgistConst.DEFAULT_CHARSET);
		final var request = this.builder
	 ...
	 * @throws NetException 网络异常
	 */
	public HttpHeaders head() throws NetException {
		final var request = this.builder
	...
	public static final <T> HttpResponse<T> get(String url, HttpResponse.BodyHandler<T> handler) throws NetException {
		return get(url, handler, CONNECT_TIMEOUT, RECEIVE_TIMEOUT);
	}
