...
void guiguzi::libtorch_face_detection(const std::string& model, const std::string& path) {
    ...
    yolo_model = torch::jit::load(model);
    yolo_model.eval();
    ...
    auto boxes = keep.index({torch::indexing::Slice(), torch::indexing::Slice(torch::indexing::None, 4)});
    keep.index_put_({torch::indexing::Slice(), torch::indexing::Slice(torch::indexing::None, 4)}, scale_boxes({input_image.rows, input_image.cols}, boxes, {image.rows, image.cols}));
