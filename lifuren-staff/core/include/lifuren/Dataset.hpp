...
public:
    ...
    // part -> staff -> note
    std::map<std::string, std::map<int, std::vector<Note>>> partMap; // 多声部五线谱音符列表
...
/**
 * @param right_staff 右手谱面索引
 * @param source      原始文件路径
 * @param target      保存文件路径
 * @param fingerings  指法：左手|右手
 * 
 * @return 是否成功
 */
extern bool save_xml(const int right_staff, const std::string& source, const std::string& target, const std::vector<std::vector<int>>& fingerings);
