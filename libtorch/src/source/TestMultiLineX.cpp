...
int main() {
    ...
    // 不计算degree
    auto x = torch::randn({1000, 1}) * 5;
    auto features = torch::cat({x, x.pow(2), x.pow(3)}, 1);
    auto labels = features.select(1, 0) * 6.2 + features.select(1, 1) * 3.6 + features.select(1, 2) * 1.6 + 10 + (torch::randn({1000}) * 40);
    ...
    std::vector<double> xv(xp, xp + 1000);
    std::vector<double> yv(yp, yp + 1000);
