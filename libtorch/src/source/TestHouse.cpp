...
int main() {
    ...
    auto tensor = torch::from_blob(mark.data(), { rows, cols });
    ...
    std::cout << model->forward(features[0].squeeze(0)).item<long long>() << '\n';
    std::cout << model->forward(features[1].squeeze(0)).item<long long>() << '\n';
    std::cout << model->forward(testTensor[0].squeeze(0)).item<long long>() << '\n';
    std::cout << model->forward(testTensor[1].squeeze(0)).item<long long>() << '\n';
    return 0;
}
