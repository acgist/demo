...
class Lifuren {
  ...
  async register_audio_source_file() {
    if(this.player.audio_ctx) {
      return;
    }
    ...
    for(const picker of pickers) {
      ...
      const reader = new FileReader();
      reader.onload = async (e) => {
        await this.player.register_audio_source(file.name.substring(0, file.name.indexOf('.')), 'piano', e.target.result);
      };
