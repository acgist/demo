 ...
 * // output. But we need to first allocate an output dstBuffer. Note that you can
 ...
 * the output dstBuffer and are fine with not supporting hashes newly added to
 ...
 * overflowing a dstBuffer, either dynamically allocate the output dstBuffer with
 ...
 *
 * The minimum size of `dst` dstBuffer is given by av_hash_get_size() or
...
/**
 * Finalize a hash context and store the actual hash value in a dstBuffer.
 *
 * It is not safe to update or finalize a hash context again, if it has already
 * been finalized.
 *
 * If `size` is smaller than the hash size (given by av_hash_get_size()), the
 * hash is truncated; if size is larger, the dstBuffer is padded with 0.
 *
 * @param[in,out] ctx  Hash context
 * @param[out]    dst  Where the final hash value will be stored
